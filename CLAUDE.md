...
- **In progress / mid-task:** **Root-causing "junky audio" on iOS — stop guessing.** Three prior blind fixes (648b168 preview-decode hang, 4a56208 `.ambient`→`.playback` session, 8dca22c bundle samples) never confirmed the real cause: smoke tests are headless Chromium and can't repro iOS WebKit audio. Static analysis: the whole asset pipeline is sound on paper (samples valid MPEG, build copies→www, Codemagic build-before-sync, webDir=www, ./samples resolves under capacitor://localhost) → it's a device-only runtime fact. **User symptom: "tinny/brash/plucky" = Karplus-Strong fallback** (`_playKSNote`), which fires only when preview `decodeAudioData` doesn't populate `_guitarBufs`; the Play-tab guitar `return`s SILENT (~line 2725) when ITS decode fails, so "both off" = the shared sample-decode step failing on device. **Leading hypothesis: iOS AudioToolbox rejects the mp3s where Chromium's lenient decoder accepts → green tests, junk on device.**
- **Active branch:** landed on `main` (commit aca0949; also pushed `claude/audio-quality-latest-build-4a2y3z`).
- **Recent decisions / changes worth remembering:** Shipped an **on-device audio self-test** (on main → next Codemagic build). Hidden + read-only, opened by **long-press (700ms) on the header title "Jazz Guitar Lab."** Runs the real fetch→decodeAudioData chain on-device: per-file HTTP status/bytes + decode pass/fail WITH error, AudioContext state/sampleRate, live preview decode counts (X/12), A/B sampled-vs-KS playback, Copy-report, and a one-line **verdict** (fetch fails→not bundled; decode fails→device rejects mp3s [root cause]; all pass→session/routing). Code: `runAudioSelfTest()` + `AudioDiagSheet` near line 644.
- **Next concrete step:** **User runs the self-test on the next TestFlight build and reads back the verdict.** Then act on it: DECODE FAILS → re-encode samples to a format iOS reliably decodes (16-bit WAV or AAC-m4a) + re-test; FETCH FAILS → fix bundling/path; all pass → investigate AVAudioSession/routing/mix. **Do NOT ship another blind audio fix before the verdict.**
- **Open questions for the user:** Waiting on the self-test verdict from the device.
