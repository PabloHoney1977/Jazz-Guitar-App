...
// ── EarTrainingView ───────────────────────────────────────────────────
// ── Fretboard trainer ─────────────────────────────────────────────────
// Visual/kinesthetic complement to EarTrainingView: builds note-name and
// (later) interval fluency on the neck. Free tier = natural notes, frets 0–5.
const FT_MAX_FRET=5;
const FT_NATURAL=new Set([0,2,4,5,7,9,11]);
const FT_NAT_NAMES=['C','D','E','F','G','A','B'];
const FT_NAT_PCS=[0,2,4,5,7,9,11];
const FT_OK='#3fa15a', FT_NO='#d65b5b';
const ftIsNatural=pc=>FT_NATURAL.has(((pc%12)+12)%12);
const ftName=pc=>N_SHARP[((pc%12)+12)%12]; // naturals only in free tier → unambiguous
function ftPositions(maxFret){
  const out=[];
  for(let s=0;s<6;s++)for(let f=0;f<=maxFret;f++){
    const pc=(OPEN_PC[s]+f)%12;
    if(ftIsNatural(pc)) out.push({s,f,pc});
  }
  return out;
}
function playNoteMidi(midi,vol,decay){
  try{const ctx=_getPreviewCtx();if(!ctx)return;
    if(_guitarBufs)_playSampledNote(ctx,midi,ctx.currentTime+0.04,vol||0.6,decay||2.0);
    else _playKSNote(ctx,midi,ctx.currentTime+0.04,(vol||0.6)*0.9);
  }catch(ex){}
}
// Mastery tier from a {r,w} record: 0 untested · 1 seen/learning · 2 mastered
function ftTier(rec){
  if(!rec||(rec.r+rec.w)===0) return 0;
  if(rec.r>=3 && rec.r/(rec.r+rec.w)>=0.8) return 2;
  return 1;
}

// ── DrillNeck ─────────────────────────────────────────────────────────
// Interactive neck for the fretboard trainer. Open column + `maxFret`
// fretted frets, every cell tappable with a ≥44px hit area + ARIA. Per-cell
// appearance is driven by cellState(stringIdx,fret).
function DrillNeck({maxFret,cellState,onTap}){
  maxFret=maxFret||5;
  const NF=maxFret;
  const FW=52,SH=44,PL=40,PT=24,PB=30;
  const W=PL+NF*FW+24,H=PT+5*SH+PB;
  const nx=wf=>PL+(wf-0.5)*FW;
  const OPEN_X=PL-15;
  const sy=si=>PT+(5-si)*SH;
  const tap=(si,fret)=>{ if(!onTap) return; playNoteMidi(OPEN_MIDI[si]+fret,0.5,1.6); onTap(si,fret); };
  function cell(si,fret,cx,cy,ck){
    const st=(cellState&&cellState(si,fret))||{};
    const name=ftName((OPEN_PC[si]+fret)%12);
    const lbl=st.label!=null?st.label:name;
    return e('g',{key:ck,onClick:()=>tap(si,fret),
      role:'button','aria-label':'String '+STR_NAMES[si]+', fret '+fret+', '+name,
      tabIndex:onTap?0:-1,
      onKeyDown:ev=>{if(onTap&&(ev.key==='Enter'||ev.key===' ')){ev.preventDefault();tap(si,fret);}},
      style:{cursor:onTap?'pointer':'default',opacity:st.dim?0.22:1,outline:'none'}},
      e('rect',{x:cx-FW*0.48,y:cy-SH*0.46,width:FW*0.96,height:SH*0.92,fill:'transparent',pointerEvents:'all'}),
      st.dot
        ?e('g',{filter:st.glow?'url(#drillneck-g)':null,style:st.pulse?{animation:'barPulse 0.9s ease-in-out infinite'}:null},
            e('circle',{cx,cy,r:st.r||13,fill:st.fill||'var(--gold)',stroke:st.stroke||'var(--hi-dot-str)',strokeWidth:1.8}),
            lbl!==''?e('text',{x:cx,y:cy,textAnchor:'middle',dominantBaseline:'middle',
              fill:st.txt||'var(--dot-lbl)',fontSize:st.fs||9,fontWeight:'bold',fontFamily:UI_FONT,
              pointerEvents:'none',style:{userSelect:'none'}},lbl):null
          )
        :e('circle',{cx,cy,r:st.r||7,fill:st.fill||'transparent',
            stroke:st.stroke||'rgba(255,255,255,0.16)',strokeWidth:1})
    );
  }
  return e('svg',{width:'100%',viewBox:`0 0 ${W} ${H}`,
    style:{display:'block',WebkitTapHighlightColor:'transparent',touchAction:'manipulation'}},
    e('defs',null,
      e('filter',{id:'drillneck-g',x:'-60%',y:'-60%',width:'220%',height:'220%'},
        e('feGaussianBlur',{stdDeviation:'3',result:'b'}),
        e('feMerge',null,e('feMergeNode',{in:'b'}),e('feMergeNode',{in:'SourceGraphic'}))),
      e('linearGradient',{id:'drillneck-bg',x1:'0',y1:'0',x2:'1',y2:'0'},
      ...
      e('linearGradient',{id:'drillneck-nut',x1:'0',y1:'0',x2:'0',y2:'1'},
        e('stop',{offset:'0%',stopColor:'#e8c870'}),
        e('stop',{offset:'100%',stopColor:'#b8922a'}))
    ),
    e('rect',{x:PL-22,y:PT-13,width:NF*FW+28,height:5*SH+26,rx:7,fill:'url(#drillneck-bg)'}),
    e('rect',{x:PL-22,y:PT-13,width:NF*FW+28,height:5*SH+26,rx:7,fill:'none',style:{stroke:'var(--neck-edge)'},strokeWidth:1}),
    e('rect',{x:PL-9,y:PT-11,width:8,height:5*SH+22,fill:'url(#drillneck-nut)',rx:2}),
    ...Array.from({length:NF},(_,k)=>e('line',{key:'fl'+k,
      x1:PL+(k+1)*FW,y1:PT-10,x2:PL+(k+1)*FW,y2:PT+5*SH+10,style:{stroke:'var(--neck-fret)'},strokeWidth:1.5})),
    ...[3,5].filter(f=>f<=NF).map(f=>e('circle',{key:'inlay'+f,cx:nx(f),cy:PT+2.5*SH,r:5,fill:'var(--neck-inlay)'})),
    ...Array.from({length:6},(_,si)=>e('line',{key:'sl'+si,
      x1:PL-22,y1:sy(si),x2:PL+NF*FW+8,y2:sy(si),stroke:`rgba(220,195,130,${0.30+si*0.09})`,strokeWidth:0.4+si*0.26})),
    ...Array.from({length:NF},(_,k)=>e('text',{key:'fn'+k,
      x:nx(k+1),y:H-9,textAnchor:'middle',style:{fill:'var(--neck-lbl)'},fontSize:11,fontFamily:UI_FONT},k+1)),
    ...STR_NAMES.map((n,si)=>e('text',{key:'sn'+si,x:PL-26,y:sy(si),
      textAnchor:'end',dominantBaseline:'middle',style:{fill:'var(--neck-lbl)'},fontSize:9.5,fontFamily:UI_FONT},n)),
    ...Array.from({length:6},(_,si)=>cell(si,0,OPEN_X,sy(si),'op'+si)),
    ...Array.from({length:NF},(_,wf)=>Array.from({length:6},(_,si)=>cell(si,wf+1,nx(wf+1),sy(si),'fp'+wf+'-'+si))).flat()
  );
}

// ── FretboardView ─────────────────────────────────────────────────────
function FretboardView({level,onPracticed,onUpgrade}){
  const SESSION_LEN=10;
  const [mode,setMode]=useState(()=>safeLS('jg-fret-mode','name')); // name|find|explore
  const [posDetail,setPosDetail]=useState(()=>{try{return JSON.parse(safeLS('jg-fret-pos','{}'))||{};}catch(ex){return {};}});
  const [assist,setAssist]=useState(()=>safeLS('jg-fret-assist','1')==='1');
  const [sound,setSound]=useState(()=>safeLS('jg-fret-sound','1')==='1');
  const [expRoot,setExpRoot]=useState(null);

  const [phase,setPhase]=useState('drill'); // drill|results
  const [answered,setAnswered]=useState(0);
  const [sessR,setSessR]=useState(0);
  const [current,setCurrent]=useState(null); // {s,f,pc}
  const [revealed,setRevealed]=useState(false);
  const [lastResult,setLastResult]=useState(null);
  const [wrongTap,setWrongTap]=useState(null); // {s,f}
  const [choices,setChoices]=useState([]);

  const practicedRef=useRef(false);
  const positions=useMemo(()=>ftPositions(FT_MAX_FRET),[]);

  useEffect(()=>{safeLSSet('jg-fret-pos',JSON.stringify(posDetail));},[posDetail]);
  useEffect(()=>{safeLSSet('jg-fret-mode',mode);},[mode]);
  useEffect(()=>{safeLSSet('jg-fret-assist',assist?'1':'0');},[assist]);
  useEffect(()=>{safeLSSet('jg-fret-sound',sound?'1':'0');},[sound]);

  const keyOf=p=>p.s+'-'+p.f;
  const playPos=(p,vol)=>{ if(sound) playNoteMidi(OPEN_MIDI[p.s]+p.f,vol||0.6,2.0); };

  // Spaced repetition: ~50% draw from weak/untested positions, else any.
  function pickPos(){
    const weak=positions.filter(p=>{const r=posDetail[keyOf(p)];const t=r?r.r+r.w:0;return t===0||r.r/t<0.8;});
    const pool=(weak.length&&Math.random()<0.5)?weak:positions;
    let p=pool[Math.floor(Math.random()*pool.length)];
    for(let i=0;i<4&&current&&p.s===current.s&&p.f===current.f;i++) p=pool[Math.floor(Math.random()*pool.length)];
    return p;
  }
  function nextQuestion(){
    const p=pickPos();
    setCurrent(p);setRevealed(false);setLastResult(null);setWrongTap(null);
    if(mode==='name'){
      const correct=ftName(p.pc);
      const others=FT_NAT_NAMES.filter(n=>n!==correct).sort(()=>Math.random()-0.5).slice(0,3);
      setChoices([correct,...others].sort(()=>Math.random()-0.5));
    }
    setTimeout(()=>{ if(sound) playNoteMidi(OPEN_MIDI[p.s]+p.f,0.6,2.0); },140);
  }
  function startSession(){ setPhase('drill');setAnswered(0);setSessR(0);nextQuestion(); }

  // (Re)start a session whenever the drill mode changes (not Explore).
  useEffect(()=>{ if(mode!=='explore') startSession(); /* eslint-disable-next-line */ },[mode]);

  function record(p,correct){
    setPosDetail(d=>{const r={...(d[keyOf(p)]||{r:0,w:0})};r[correct?'r':'w']++;return {...d,[keyOf(p)]:r};});
    setRevealed(true);setLastResult(correct?'right':'wrong');
    setSessR(x=>x+(correct?1:0));
    setAnswered(a=>{
      const n=a+1;
      if(n>=SESSION_LEN&&!practicedRef.current){practicedRef.current=true;onPracticed&&onPracticed();}
      return n;
    });
  }
  function guessName(name){
    if(revealed||!current) return;
    record(current,name===ftName(current.pc));
    playPos(current,0.6);
  }
  function tapFind(s,f){
    if(mode!=='find'||revealed||!current) return;
    const correct=s===current.s&&((OPEN_PC[s]+f)%12)===current.pc;
    if(!correct) setWrongTap({s,f});
    record(current,correct);
  }
  function onNext(){ if(answered>=SESSION_LEN) setPhase('results'); else nextQuestion(); }

  // Desktop keyboard: number keys pick a choice in Name mode; Enter advances.
  useEffect(()=>{
    const h=ev=>{
      if(phase!=='drill'||mode!=='name') return;
      if(revealed){ if(ev.key==='Enter'){ev.preventDefault();onNext();} return; }
      const n=parseInt(ev.key,10);
      if(n>=1&&n<=choices.length){ev.preventDefault();guessName(choices[n-1]);}
    };
    window.addEventListener('keydown',h);
    return ()=>window.removeEventListener('keydown',h);
  });

  // Weakest natural position for the results card.
  const weakest=(()=>{
    let worst=null,worstRate=1;
    positions.forEach(p=>{const r=posDetail[keyOf(p)];const t=r?r.r+r.w:0;if(t<2)return;
      const rate=r.r/t;if(rate<worstRate){worstRate=rate;worst={p,r:r.r,w:r.w};}});
    return worst;
  })();
  const learned=positions.filter(p=>ftTier(posDetail[keyOf(p)])>=1).length;
  const mastered=positions.filter(p=>ftTier(posDetail[keyOf(p)])>=2).length;

  // ── cellState builders ──
  function csDrill(s,f){
    const pc=(OPEN_PC[s]+f)%12;
    if(mode==='name'){
      if(current&&s===current.s&&f===current.f)
        return {dot:true,fill:GOLD,glow:true,label:revealed?ftName(pc):'?',txt:'var(--dot-lbl)'};
      return {};
    }
    // find
    if(revealed&&current&&s===current.s&&pc===current.pc)
      return {dot:true,fill:FT_OK,glow:true,label:ftName(pc)};
    if(wrongTap&&s===wrongTap.s&&f===wrongTap.f)
      return {dot:true,fill:FT_NO,label:ftName(pc)};
    if(assist&&!revealed&&current&&s!==current.s) return {dim:true};
    return {};
  }
  function csExplore(s,f){
    const pc=(OPEN_PC[s]+f)%12;
    if(!ftIsNatural(pc)) return {};
    if(expRoot!=null){
      const iv=((pc-expRoot)+12)%12;
      return {dot:true,r:11,fs:8,fill:pc===expRoot?GOLD:'rgba(116,192,252,0.30)',
        stroke:'rgba(255,255,255,0.25)',label:INT_NAMES[iv],txt:'var(--dot-lbl)'};
    }
    return {dot:true,r:11,fs:8,fill:'rgba(255,255,255,0.06)',stroke:'rgba(255,255,255,0.22)',
      label:ftName(pc),txt:'var(--txt)'};
  }
  function csHeat(s,f){
    const pc=(OPEN_PC[s]+f)%12;
    if(!ftIsNatural(pc)) return {};
    const tier=ftTier(posDetail[s+'-'+f]);
    if(tier===0) return {dot:true,r:6,fill:'rgba(255,255,255,0.05)',stroke:'rgba(255,255,255,0.12)',label:''};
    if(tier===1) return {dot:true,r:10,fs:8,fill:'rgba(212,168,85,0.55)',stroke:'rgba(212,168,85,0.8)',label:ftName(pc),txt:'#1a1a2e'};
    return {dot:true,r:11,fs:8,fill:'rgba(63,161,90,0.85)',stroke:'#2f7d49',label:ftName(pc),txt:'#fff'};
  }

  const segBtn=(id,lbl)=>e('button',{key:id,onClick:()=>setMode(id),style:{
    flex:1,padding:'8px',borderRadius:8,cursor:'pointer',fontFamily:UI_FONT,fontSize:'0.82rem',
    fontWeight:mode===id?700:400,border:'1px solid '+(mode===id?GOLD:BTN_BRD),
    background:mode===id?ACT_GOLD:'transparent',color:mode===id?GOLD:BTN_OFF,minHeight:42}},lbl);
  const toggle=(on,setOn,lbl)=>e('button',{onClick:()=>setOn(!on),style:{
    padding:'4px 10px',borderRadius:6,cursor:'pointer',fontFamily:UI_FONT,fontSize:'0.7rem',minHeight:32,
    border:'1px solid '+(on?GOLD:BTN_BRD),background:on?ACT_GOLD:'transparent',color:on?GOLD:BTN_OFF}},
    lbl+': '+(on?'on':'off'));

  const prompt=current?(mode==='name'
    ?'What note is the highlighted dot?'
    :'Find '+ftName(current.pc)+' on the '+STR_NAMES[current.s]+' string'):'';
  const statusText=revealed?(lastResult==='right'?'Correct':'Not quite')
    +(current?' — '+ftName(current.pc):''):'';

  return e('div',null,
    // Mode segmented control
    e('div',{'data-tour':'fretboard-modes',style:{display:'flex',gap:8,marginBottom:12}},
      segBtn('name','Name the Note'),segBtn('find','Find the Note'),segBtn('explore','Explore')),

    // Aria-live status for screen readers
    e('div',{'aria-live':'polite',style:{position:'absolute',width:1,height:1,overflow:'hidden',clip:'rect(0 0 0 0)'}},statusText),

    mode==='explore'
      ?e('div',null,
        e('div',{style:{fontSize:'0.72rem',color:HINT,fontFamily:UI_FONT,marginBottom:8}},
          'Tap any note to hear it. Pick a root to see every note as an interval from it.'),
        e('div',{style:{display:'flex',gap:4,flexWrap:'wrap',alignItems:'center',marginBottom:10}},
          e('span',{style:{fontSize:'0.72rem',color:LBL}},'Root'),
          e('button',{onClick:()=>setExpRoot(null),style:{padding:'3px 9px',borderRadius:5,cursor:'pointer',
            fontFamily:UI_FONT,fontSize:'0.72rem',minHeight:32,
            border:'1px solid '+(expRoot==null?GOLD:BTN_BRD),background:expRoot==null?ACT_GOLD:'transparent',
            color:expRoot==null?GOLD:BTN_OFF}},'Notes'),
          ...FT_NAT_NAMES.map((n,i)=>{const pc=FT_NAT_PCS[i];
            return e('button',{key:n,onClick:()=>setExpRoot(pc),style:{padding:'3px 9px',borderRadius:5,cursor:'pointer',
              fontFamily:UI_FONT,fontSize:'0.72rem',minHeight:32,
              border:'1px solid '+(expRoot===pc?GOLD:BTN_BRD),background:expRoot===pc?ACT_GOLD:'transparent',
              color:expRoot===pc?GOLD:BTN_OFF}},n);})
        ),
        e('div',{style:{background:'var(--neck-wrap)',border:'1px solid '+BORDER,borderRadius:9,padding:'6px 4px',marginBottom:10}},
          e(DrillNeck,{maxFret:FT_MAX_FRET,cellState:csExplore,onTap:()=>{}}))
      )
      :phase==='results'
      ?e('div',{style:{textAlign:'center',padding:'10px 4px 16px'}},
        e('div',{style:{fontSize:'2rem',marginBottom:6,animation:'doneFlash 0.6s ease'}},'✓'),
        e('div',{style:{fontFamily:SERIF,fontSize:'1.3rem',fontWeight:700,marginBottom:4}},
          'Session: '+sessR+' / '+SESSION_LEN),
        e('div',{style:{fontSize:'0.8rem',color:HINT,marginBottom:weakest?6:16}},
          Math.round(100*sessR/SESSION_LEN)+'% this round'),
        weakest?e('div',{style:{fontSize:'0.78rem',color:LBL,marginBottom:16}},
          'Work on: ',e('b',null,ftName(weakest.p.pc)+' on the '+STR_NAMES[weakest.p.s]+' string')):null,
        e('div',{style:{display:'flex',gap:10,justifyContent:'center'}},
          e('button',{onClick:startSession,style:{padding:'10px 22px',borderRadius:8,fontSize:'0.9rem',
            fontWeight:700,background:GOLD,color:'#000',border:'none',cursor:'pointer',minHeight:44}},'Done ✓'),
          e('button',{onClick:()=>{setPhase('drill');setAnswered(0);setSessR(0);nextQuestion();},
            style:{padding:'10px 22px',borderRadius:8,fontSize:'0.9rem',fontWeight:600,background:'transparent',
            color:GOLD,border:'1px solid '+GOLD,cursor:'pointer',minHeight:44}},'Keep going +10'))
      )
      :e('div',null,
        // Progress counter + prompt
        e('div',{style:{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8,gap:8,flexWrap:'wrap'}},
          e('span',{style:{fontSize:'0.9rem',fontWeight:600,fontFamily:UI_FONT,color:'var(--txt)'}},prompt),
          e('span',{style:{fontSize:'0.78rem',color:HINT,fontFamily:UI_FONT,whiteSpace:'nowrap'}},
            Math.min(answered+1,SESSION_LEN)+' / '+SESSION_LEN)),
        // Neck
        e('div',{style:{background:'var(--neck-wrap)',border:'1px solid '+BORDER,borderRadius:9,padding:'6px 4px',marginBottom:10}},
          e(DrillNeck,{maxFret:FT_MAX_FRET,cellState:csDrill,onTap:mode==='find'?tapFind:()=>{}})),
        // Name-mode choices
        mode==='name'?e('div',{style:{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8,marginBottom:10}},
          choices.map((nm)=>{const isAns=revealed&&current&&nm===ftName(current.pc);
            return e('button',{key:nm,onClick:()=>guessName(nm),disabled:revealed,style:{
              padding:'12px 6px',borderRadius:8,cursor:revealed?'default':'pointer',fontFamily:SERIF,
              fontSize:'1.05rem',fontWeight:700,minHeight:52,
              border:'2px solid '+(isAns?GOLD:BTN_BRD),background:isAns?ACT_YEL:BG2,
              color:isAns?GOLD:BTN_OFF,opacity:revealed&&!isAns?0.5:1}},nm);})
        ):null,
        // Find-mode hint
        mode==='find'?e('div',{style:{fontSize:'0.72rem',color:HINT,fontFamily:UI_FONT,textAlign:'center',marginBottom:10}},
          'Tap the note on the fretboard'):null,
        // Reveal banner + next
        revealed?e('div',{style:{display:'flex',alignItems:'center',justifyContent:'space-between',gap:10,
          padding:'8px 12px',borderRadius:8,marginBottom:10,
          background:lastResult==='right'?ACT_TEAL:ACT_RED,
          border:'1px solid '+(lastResult==='right'?FT_OK:FT_NO)}},
          e('span',{style:{fontSize:'0.82rem',fontWeight:600,color:lastResult==='right'?FT_OK:FT_NO}},
            lastResult==='right'?'✓ Correct'
              :'✗ That was '+(wrongTap?ftName((OPEN_PC[wrongTap.s]+wrongTap.f)%12)+' — ':'')
                +ftName(current.pc)+(mode==='find'?' is shown in green':'')),
          e('button',{onClick:onNext,style:{padding:'7px 18px',borderRadius:6,fontSize:'0.85rem',fontWeight:700,
            background:GOLD,color:'#000',border:'none',cursor:'pointer',minHeight:40,flexShrink:0}},
            answered>=SESSION_LEN?'See results →':'Next →')
        ):null,
        // Toggles
        e('div',{style:{display:'flex',gap:8,flexWrap:'wrap',marginBottom:14}},
          toggle(sound,setSound,'Sound'),
          mode==='find'?toggle(assist,setAssist,'Assist'):null)
      ),

    // Mastery heatmap (always visible — the persistent progress map)
    e('div',{style:{borderTop:'1px solid '+BORDER,paddingTop:12}},
      e('div',{style:{display:'flex',alignItems:'baseline',justifyContent:'space-between',marginBottom:6,flexWrap:'wrap',gap:6}},
        e('span',{style:{fontSize:'0.74rem',color:LBL,letterSpacing:'0.3px',fontWeight:600}},'Your fretboard'),
        e('span',{style:{fontSize:'0.72rem',color:HINT,fontFamily:UI_FONT}},
          'Learned '+learned+' / '+positions.length+'  ·  Mastered '+mastered)),
      e('div',{style:{background:'var(--neck-wrap)',border:'1px solid '+BORDER,borderRadius:9,padding:'6px 4px'}},
        e(DrillNeck,{maxFret:FT_MAX_FRET,cellState:csHeat,onTap:null})),
      e('div',{style:{fontSize:'0.66rem',color:HINT,fontFamily:UI_FONT,marginTop:6,textAlign:'center'}},
        'Natural notes, frets 0–5 · grey = new · amber = seen · green = mastered')
    )
  );
}

// ── TrainView ─────────────────────────────────────────────────────────
// Hub for the "Train" tab: Ear training + Fretboard trainer as sibling modes.
function TrainView({level,onPracticed,onUpgrade,pedalRef}){
  const [fam,setFam]=useState(()=>safeLS('jg-train-fam','ear')); // ear|fretboard
  useEffect(()=>{safeLSSet('jg-train-fam',fam);},[fam]);
  const famBtn=(id,lbl)=>e('button',{key:id,'data-tour':'train-'+id,onClick:()=>setFam(id),style:{
    flex:1,padding:'9px',borderRadius:8,cursor:'pointer',fontFamily:UI_FONT,fontSize:'0.86rem',
    fontWeight:fam===id?700:500,border:'1px solid '+(fam===id?'var(--txt)':BTN_BRD),
    background:fam===id?'var(--bg2)':'transparent',color:fam===id?'var(--txt)':BTN_OFF,minHeight:44}},lbl);
  return e('div',null,
    e('div',{style:{display:'flex',gap:8,marginBottom:14}},
      famBtn('ear','♫ Ear'),famBtn('fretboard','◉ Fretboard')),
    fam==='ear'
      ?e(EarTrainingView,{level,onPracticed,onUpgrade,pedalRef})
      :e(FretboardView,{level,onPracticed,onUpgrade})
  );
}
...
function GuideView({openPreset,level,streak,lastPracticeDay,bestStreak,onUpgrade,onPracticed}){
  ...
  const stages=[
    ...
    {id:'drop2a',phase:'Voicings',phaseLabel:'Phase 2 — Voicings & Texture',fullPreset:true,
     ...
     body:[[term('drop2','Drop 2'),' takes a "closed" chord (all 4 notes within one octave) and drops the second-highest note down an octave. This spreads the chord across 4 adjacent strings in a span the hand can reach. Drop 2 is the standard jazz guitar comping voicing.'],
     ...
     items:['On strings 1–4: find Drop 2 root position for Cmaj7, Dm7, G7, and Bm7♭5','Learn all 4 inversions of Cmaj7 on strings 1–4 — play them low to high and back','Do the same for Dm7 and G7 — now you have 12 shapes for a ii–V–I','Play each shape cleanly at 50 BPM — accuracy first, speed follows','Done when: all 4 qualities, all 4 inversions on strings 1–4, no hesitation on any shape']},
