      ...
      .map(({chord,ci})=>{
        ...
        return e('div',{key:ci,style:{flex:'1 1 200px',minWidth:190}},
          ...
          e('div',{style:{display:'flex',gap:6,flexWrap:'wrap'}},
            vType==='shell'
              ?SHELLS.map((sh,ii)=>
                  e(ChordBox,{key:ii,voicing:voicings[ii],strings:sh.s,tones:chord.tones,
                    degNames:chord.dnames,invLabel:sh.lbl+' ('+sh.root+')',
                    bassLabel:'bass: R',
...
function GuideView({openPreset,level}){
  ...
  const stages=[
    {id:'qualities',title:'Meet the four chord qualities',
     ...
     items:['In the app, open Chords and click through I, II, V and VII — one chord of each quality','Set the dot mode to "Interval" — the colored dots show the four chord tones: Root (red), 3rd (teal), 5th (blue), 7th (gold). These same colors appear everywhere in the app','Listen: can you name the quality before you read the label?']},
